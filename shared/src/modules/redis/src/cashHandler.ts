...
export class CashHandler {
  ...
  private writeEntityToRedis({ type, entityId, mainFieldId, entFields }: RedisQuerySet) {
    const arr = CashHandler.redisInputIntoArrayString(entFields);
    ...
    if (arr && arr.length) {
      arr.forEach((answer) => {
        if (answer.argsToResolve && answer.argsToResolve.type === RedisGetTypes.hashMap) {
          ...
          newMissionAnswer[mapSetFieldsValue.mainField] = CashHandler.fromRedisArrayToObject(answer.response);
        }
      });
    }
  ...
  public getMission({ type, missionId, mainFieldId }: RedisQueryGetMission, max, min) {
    return new Promise((resolve, reject) => {
      ...
      this.redis.getRangeSetByScoreHighToLow(redisKey,
          ...
          .then((res: any) => {
            // console.log(res);
            const mission = CashHandler.fromRedisArrayToObject(res);
            resolve(mission);
          });
      this.redis.execData()
          .catch((err) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.redis.execData()
          .then(() => {
            prom.then((res) => {
              console.log(res);
              const stretcheredObject = CashHandler.fromRedisArrayToObject(res);
